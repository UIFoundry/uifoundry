...
<step number="6" name="cleanup_trial_changes">

### Step 6: Cleanup Trial-and-Error Changes (CRITICAL)

Remove temporary or experimental changes introduced during troubleshooting so only the final solution remains.

<instructions>
  ACTION: Remove or revert all temporary artifacts and debug scaffolding
  INCLUDE:
    - Delete duplicate or experimental test files/specs created during investigation
    - Remove readiness checks, sleep/retry loops, and noisy console logs that aren’t required post-solution
    - Revert temporary workflow edits (e.g., narrowing to a subset of tests) back to the intended defaults
    - Eliminate dead code, unused helpers/scripts, or commented-out blocks added for debugging
    - Ensure configuration/env usage matches the final approach (e.g., a single base URL env)
  VERIFY:
    - Full test suite passes after cleanup
    - CI workflows run the correct scope (all tests unless intentionally limited)
</instructions>
