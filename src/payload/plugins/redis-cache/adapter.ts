...
export function wrapAdapterWithCache(
  ...
  async function invalidateByPattern(pattern: string): Promise<void> {
    try {
      ...
      if (keys.length > 0) {
        ...
        debugLog(config, `Cache INVALIDATED: ${keys.length} keys matching ${pattern}`)
      }
    } catch (error) {
  ...
  return {
     ...
     */
    update: async (args) => {
      const result = await baseAdapter.update(args)
     ...
     */
    delete: async (args) => {
      const result = await baseAdapter.delete(args)
